...
namespace PathTracer
{
    ...
    struct PathTrace::PathTraceImpl
    {
    ...
    PathTrace::PathTrace(std::shared_ptr<VulkanManager> manager)
        : manager_(manager)
        , impl_(std::make_unique<PathTraceImpl>(manager))
    {
    }
    ...
    void PathTrace::Init(Scene const& scene, RrAccelerationStructure top_level_structure, RrContext context, uint32_t num_rays)
    {
    ...
    void PathTrace::PrepareCommandBuffer(uint32_t num_rays)
    {
        ...
        impl_->pt_rays_pipeline_.Create("shaders/pt_rays.comp.spv", {
            { VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, 1},  // Params
            ...
            { VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, (uint32_t)impl_->image_infos_.size()} // Textures
                                        });
        impl_->pt_rays_resolve_pipeline_.Create("shaders/pt_rays_resolve.comp.spv", {
            { VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, 1},  // AoBuffer
            { VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, 1},  // Color
            { VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, 1},  // Ids
            { VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, 1},  // RayCount
        ...
        VkDescriptorSet pt_rays_desc = impl_->pt_rays_pipeline_.Bind(pt_ray_all_binding);
        vkCmdBindDescriptorSets(cmd_buf, VK_PIPELINE_BIND_POINT_COMPUTE, impl_->pt_rays_pipeline_.GetPipelineLayout(), 0, 1, &pt_rays_desc, 0, nullptr);
        vkCmdDispatch(cmd_buf, (num_rays + 63) / 64, 1u, 1u);

        VkBuffer before_pt_rays_trace[3] = { impl_->ao_rays_.get() , impl_->ao_count_.get(), impl_->color_.get() };
        manager_->EncodeBufferBarriers(before_pt_rays_trace, 3, VK_ACCESS_SHADER_WRITE_BIT,
                                       VK_ACCESS_SHADER_READ_BIT,
                                       VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT,
                                       VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT, cmd_buf);
        ...
        //    impl_->hits_.get(),
        //};
        //VkDescriptorSet pt_rays_resolve_desc = impl_->pt_rays_resolve_pipeline_.Bind(pt_color_ray_binding);
        //vkCmdBindDescriptorSets(cmd_buf, VK_PIPELINE_BIND_POINT_COMPUTE, impl_->pt_rays_resolve_pipeline_.GetPipelineLayout(), 0, 1, &pt_rays_resolve_desc, 0, nullptr);
        //vkCmdDispatch(cmd_buf, (num_rays + 63) / 64, 1u, 1u);
    ...
    std::vector<uint32_t> PathTrace::GetColor() const
    {
    ...
    VkBuffer PathTrace::GetColorBuffer() const
    {
        return impl_->color_.get();
    }

    void PathTrace::SetColor(std::vector<uint32_t> const& color)
    {
        impl_->WaitForFence();
        void* color_ptr = manager_->MapMemory(impl_->memory_.get(), impl_->color_offset_, impl_->color_size_);
